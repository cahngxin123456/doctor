...
@interface IWShouShuViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,UIPickerViewDelegate,UIPickerViewDataSource>
{
...
@implementation IWShouShuViewController
...
- (void)setShoushuArray:(NSArray *)shoushuArray{
    _dataSource = [NSMutableArray arrayWithCapacity:0];
    _shoushuArray = shoushuArray;
    if (shoushuArray.count == 0) {
        ...
        for (IWJiBing *model in shoushuArray) {
            [_dataSource addObject:model];
        }
    }
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    [view setSaveButtonBlock:^{
        ...
        IWAddOld2Param *param = [[IWAddOld2Param alloc] init];
        if (hiddenCell) {
            ...
            param.loginId =self.memberID;
            param.companyId = @(IWCompanyId);
            param.shoushuDatas = jsonString;
            param.noValue = @"0";
            
        }
        [MBProgressHUD showMessage:NSLocalizedString(@"save_loading", nil)];
        [IWMedicalArchivesTool addMedicalOld2WithParam:param success:^(IWBaseResult *result) {
