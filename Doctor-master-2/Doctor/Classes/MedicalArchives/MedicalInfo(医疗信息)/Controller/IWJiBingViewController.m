...
@interface IWJiBingViewController ()<UITableViewDelegate,UITableViewDataSource,UIPickerViewDelegate,UIPickerViewDataSource>
{
    BOOL hiddenCell;
}
...
@implementation IWJiBingViewController
...
- (void)setJibingArray:(NSArray *)jibingArray{
    _dataSource = [NSMutableArray arrayWithCapacity:0];
    _jibingArray = jibingArray;
    if (jibingArray.count == 0) {
        ...
        for (IWJiBing *model in jibingArray) {
            [_dataSource addObject:model];
        }
    }
...
{
    if (_tableView == nil) {
        UITableView *tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 40, IWWIDTH, IWHEIGHT - 40 -64) style:UITableViewStyleGrouped];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ...
        NSString *kong = @"";
        NSString *str1 = [NSString stringWithFormat:@"%@",model.bType?model.bType:kong];
        NSString *str2 = [NSString stringWithFormat:@"%@",model.otherType?model.otherType:kong];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ...
        vc.typeStr = @"JiBing";
        vc.selectedName = model.bType;
        __weak typeof(model)weakModel = model;
        
        [vc setDrinkTypeSaveBlock:^(NSString *str1, NSString *str2) {
            weakModel.bType = str1;
            weakModel.otherType = str2;
            [tableView reloadData];
        }];
        
        vc.textFieldtext = model.otherType;
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    [view setSaveButtonBlock:^{
        ...
        IWAddOld1Param *param = [[IWAddOld1Param alloc] init];
        if (hiddenCell) {
            ...
            param.loginId =self.memberID;
            param.companyId = @(IWCompanyId);
            param.jibingDatas = jsonString;
            param.noValue = @"0";

        }
        [MBProgressHUD showMessage:NSLocalizedString(@"save_loading", nil)];
        [IWMedicalArchivesTool addMedicalOld1WithParam:param success:^(IWBaseResult *result) {
