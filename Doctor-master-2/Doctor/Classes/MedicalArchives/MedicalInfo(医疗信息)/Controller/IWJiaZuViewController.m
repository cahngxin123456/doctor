...
@interface IWJiaZuViewController ()<UITableViewDelegate,UITableViewDataSource,UIPickerViewDelegate,UIPickerViewDataSource>
{
    BOOL hiddenCell;
}
...
@property (nonatomic, strong) IWJiaZu *currentJiaZuModel;


@end

@implementation IWJiaZuViewController
...
- (NSArray *)nameTitleArray{
    if (_nameTitleArray == nil) {
        NSArray *array = @[@"病种",@"发生时间",@"治愈时间"];
        _nameTitleArray = array;
    }
    return _nameTitleArray;
}
- (void)setMemberType:(NSString *)memberType{
    _memberType = memberType;
}
- (void)setJiazuArray:(NSArray *)jiazuArray{
    _dataSource = [NSMutableArray arrayWithCapacity:0];
    _jiazuArray = jiazuArray;
    if (jiazuArray.count == 0) {
        IWJiaZu *model = [[IWJiaZu alloc] init];
        model.diseaseType = @"";
        model.otherDiseaseType = @"";
        model.occurTime = @"";
        model.cureTime = @"";
        [_dataSource addObject:model];
    }else{
        for (IWJiaZu *model in jiazuArray) {
            [_dataSource addObject:model];
        }
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    IWJiaZu *model = self.dataSource[indexPath.section];

    cell.nameLabel.text = self.nameTitleArray[indexPath.row];
    if (indexPath.row == 0) {
        
        NSString *kong = @"";
        NSString *str1 = [NSString stringWithFormat:@"%@",model.diseaseType?model.diseaseType:kong];
        NSString *str2 = [NSString stringWithFormat:@"%@",model.otherDiseaseType?model.otherDiseaseType:kong];
        
        NSMutableString *mstr = [NSMutableString stringWithFormat:@"%@ %@",str1,str2];
        NSString *resultStr = [mstr stringByReplacingOccurrencesOfString:@" " withString:@""];
        if (resultStr.length == 0) {
            cell.resultLabel.text = @"请选择疾病种类";
        ...
        if (model.occurTime.length != 0) {
            cell.resultLabel.text = model.occurTime;
        }else{
        ...
        if (model.cureTime.length != 0) {
            cell.resultLabel.text = model.cureTime;
        }else{
            cell.resultLabel.text = @"请选择时间";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ...
        vc.navigationItem.title = NSLocalizedString(@"activity_medical_old1_type_title", nil);
        vc.typeStr = @"JiBing";
        vc.selectedName = model.diseaseType;
        __weak typeof(model)weakModel = model;
        
        [vc setDrinkTypeSaveBlock:^(NSString *str1, NSString *str2) {
            weakModel.diseaseType = str1;
            weakModel.otherDiseaseType = str2;
            [tableView reloadData];
        }];
        
        vc.textFieldtext = model.otherDiseaseType;
        [self.navigationController pushViewController:vc animated:YES];
        
    }
    
    if (indexPath.row == 1) {
        [self showPickerViewwithtitleLabele:@"发生时间" anddataArray:nil andIndexpath:indexPath];
    }
    
    if (indexPath.row == 2) {
        [self showPickerViewwithtitleLabele:@"治愈时间" anddataArray:nil andIndexpath:indexPath];
    }
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    [view setSaveButtonBlock:^{
        ...
        for (int i = 0; i<resultArray.count; i++) {
            IWJiaZu *model = resultArray[i];
            NSString *kong = @"";
            NSString *s1 = [NSString stringWithFormat:@"%@",model.diseaseType?model.diseaseType:kong];
            NSString *s2 = [NSString stringWithFormat:@"%@",model.otherDiseaseType?model.otherDiseaseType:kong];
            NSString *s3 = [NSString stringWithFormat:@"%@",model.occurTime?model.occurTime:kong];
            NSString *s4 = [NSString stringWithFormat:@"%@",model.cureTime?model.cureTime:kong];
            NSDictionary *dic = @{@"occurTime":s3,@"otherDiseaseType":s2,@"cureTime":s4,@"diseaseType":s1};
        ...
        IWAddJiaZuParam *param = [[IWAddJiaZuParam alloc] init];
        if (hiddenCell) {
            param.loginId =self.memberID;
            param.companyId = @(IWCompanyId);
            param.noValue = @"1";
            param.memberType = self.memberType;
            
        }else{
            param.loginId =self.memberID;
            param.companyId = @(IWCompanyId);
            param.jiazuDatas = jsonString;
            param.noValue = @"0";
            param.memberType = self.memberType;
        ...
        [IWMedicalArchivesTool addMedicalJiaZuWithParam:param success:^(IWBaseResult *result) {
...
- (void)showPickerViewwithtitleLabele:(NSString *)title anddataArray:(NSArray *)dataArray andIndexpath:(NSIndexPath *)index;
{
    ...
    for (int i = 0; i<self.dataSource.count; i++) {
        if (i == index.section) {
            self.currentJiaZuModel = self.dataSource[i];

            if (index.row == 1) {
                timeStr = self.currentJiaZuModel.occurTime;
            }else{
                timeStr = self.currentJiaZuModel.cureTime;
            }
        }
    }
...
- (void)timesaveButton:(UIButton *)btn
{
    ...
    if (btn.tag == 1) {
        self.currentJiaZuModel.occurTime = [NSString stringWithFormat:@"%@-%@-%@",yearStr,monthStr,dayStr];

    }else{
        self.currentJiaZuModel.cureTime = [NSString stringWithFormat:@"%@-%@-%@",yearStr,monthStr,dayStr];
