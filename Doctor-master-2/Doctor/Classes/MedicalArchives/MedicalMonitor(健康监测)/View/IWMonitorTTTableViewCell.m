...
-(void)updateConstraints{
    ...
    [_bottomView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(@(40*SFXISHU));
    }];
...
- (void)setModel:(IWBloodPressureModel *)model
{
    ...
    if (model) {
        ...
        if ([model.heartRate intValue] <=60) {
            
            [_resultView mas_makeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo(self.bottomView.mas_bottom);
                make.height.mas_equalTo(@((60-[model.heartRate intValue])*SFXISHU));
                make.width.mas_equalTo(@(130));
                make.centerX.mas_equalTo(self.bottomView.mas_centerX);
            }];

            _resultView.backgroundColor = IWColor(223, 122, 32);
            _centerLabel.attributedText = [self attributedStringWithStringOne:model.lowPressure StringOneColor:[UIColor redColor] StringOneFont:[UIFont systemFontOfSize:16] StringTwo:@"bpm" StringTwoColor:[UIColor grayColor] StringTwoFont:IWFont];
            
        }else{
            [_resultView mas_makeConstraints:^(MASConstraintMaker *make) {
                make.bottom.equalTo(self.bottomView.mas_bottom);
                make.height.mas_equalTo(@(([model.heartRate intValue]-60)*SFXISHU));
                make.width.mas_equalTo(@(130));
                make.centerX.mas_equalTo(self.bottomView.mas_centerX);
            }];
            _resultView.backgroundColor = IWColor(107, 174, 1);
            _centerLabel.text = [NSString stringWithFormat:@"%@bpm",model.heartRate];
