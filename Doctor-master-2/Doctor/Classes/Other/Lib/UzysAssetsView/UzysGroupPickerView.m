...
- (void)setupLayout
{
    ...
    self.frame = CGRectMake(0, - [UIScreen mainScreen].bounds.size.height, kWidth, [UIScreen mainScreen].bounds.size.height);;
...
- (void)setupTableView
{
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, NavigationHeight, kWidth, self.bounds.size.height -NavigationHeight) style:UITableViewStylePlain];
...
- (void)show
{
    [UIView animateWithDuration:0.25f delay:0.f options:UIViewAnimationOptionCurveEaseInOut|UIViewAnimationOptionBeginFromCurrentState animations:^{
        self.frame = CGRectMake(0, BounceAnimationPixel , kWidth, [UIScreen mainScreen].bounds.size.height);
